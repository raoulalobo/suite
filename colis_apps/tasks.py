...
def order_mofified(order_id):
    ...
    print('Coli etat : {}'.format( coli.etat_colis ) )
...
@task
def sms_coli(order_id, usr):

    coli = Coli.objects.get(id=order_id)

    if coli.etat_colis == 'envoye' :
        message = 'Cher client proprietaire du #{}, votre colis {} est en route pour {}. Montant paye : {} .Yde:699755276 ,Dla:656968928 -Plaintes ( SMS uniquement ):697509899'.format( coli.telephone_exp.phone , coli.numero_colis, coli.destination, coli.valeur_declaree)
        payload = {'action': 'sendmessage', 'username': 'FINEXS', 'password': 'Finexs12345', 'recipient': '237{}'.format(coli.telephone_exp.phone) , 'messagetype':'SMS:TEXT', 'messagedata':message}
        r = requests.get("http://api.vassarl.com:9501/api", params=payload)
    ...
    return r
    #return coli.telephone_exp.phone
